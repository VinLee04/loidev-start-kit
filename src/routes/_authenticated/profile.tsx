...
import { CarrotIcon, ShieldBanIcon, ShieldCheckIcon, SpadeIcon } from 'lucide-react';
...
function RouteComponent() {
  ...
  console.log(user)
  const onSignOut = () => authClient.signOut({
    fetchOptions: {
      onSuccess: async () => {
      ...
      <div className='relative flex flex-col p-4 md:p-10 md:border rounded-md w-full md:w-fit'>
        ...
        <div className='md:absolute top-0 left-0 md:-translate-y-1/2 flex justify-between'>
          <div className='flex gap-x-2'>
            <Link to='..' className={buttonVariants()}>Quay lại trang chủ</Link>
            <ModeToggle />
          </div>

          <div className='flex md:hidden gap-x-2'>
            <DropdownMenu>
              <DropdownMenuTrigger asChild><Button variant='secondary'>Tính năng</Button></DropdownMenuTrigger>
              <DropdownMenuContent>
                <DropdownMenuItem>Đổi mật khẩu</DropdownMenuItem>
                <DropdownMenuItem>Quên mật khẩu</DropdownMenuItem>
                <DropdownMenuSeparator />

                <DropdownMenuItem variant="destructive" disabled>Xóa tài khoản</DropdownMenuItem>
        ...
        <div className='md:flex hidden absolute bottom-0 left-0 w-full translate-y-1/2 gap-x-2'>
          <Button variant='destructive' className='mr-auto' disabled>Xóa tài khoản</Button>

          <Button variant='secondary'>Đổi mật khẩu</Button>
          <Button variant='secondary'>Quên mật khẩu</Button>
          <Button variant='default' onClick={onSignOut}>Đăng xuất</Button>
        </div>
        ...
        <div className='flex flex-col sm:flex-row gap-10 md:h-40 mt-4 md:mt-0'>
          <div className='relative md:size-40'>
            <Avatar className='size-full rounded-lg'>
              <AvatarImage src={user.image ?? 'https://api.dicebear.com/10.x/planets/svg'} />
          ...
          <div className='h-full flex flex-col gap-y-2'>
            <div className='flex items-start justify-between'>
              <div>
                <p className='text-xl font-semibold text-primary uppercase'> {user.name} </p>
                <p> {user.email} </p>
              </div>
              <Badge>
                {isAdmin ?
        ...
        <div className='flex-1 bg-muted rounded-md mt-7 p-4 grid grid-cols-2 md:grid-cols-3 gap-4 
        *:flex-col *:h-auto *:w-full *:text-base *:rounded-lg *:py-2.5
         [&>span]:text-sm [&>span>p]:font-semibold
         '>
          <Badge variant='default'>Đơn hàng
            <p> 2 </p>
          </Badge>
          <Badge variant='warning'>Đang giao
            <p> 0 </p>
          </Badge>
          <Badge variant='destructive'>Đã hủy
            <p> 0 </p>
          </Badge>
          <Badge variant='success'> Tổng tiền
            <p> 250.000 VNĐ </p>
          </Badge>
          <Badge variant='invert'>Tiết kiệm
            <p> 50.000 VNĐ </p>
          </Badge>
          <Badge variant='info'>Đã chi
            <p> 200.000 VNĐ </p>
          </Badge>
        </div>
      </div>
